...
public class PrimaryController implements Initializable, ServerAddedListener {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		if(serverList != null) {
			for(String server : serverList) {
				treeRoot.getChildren().add(makeServerTreeItem(new TreeItemData(server, ItemType.Server)));
			}
	...
	public void add(String server) {
		if(server != null && !server.isEmpty()) {
			tvLeft.getRoot().getChildren().add(makeServerTreeItem(new TreeItemData(server, ItemType.Server)));
